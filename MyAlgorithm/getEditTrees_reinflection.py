...
def getTreeAsSth(et):
  if et.data[0] == "sub":
    if et.data[1] == u'' and et.data[2] == u'':
      return []
    return [(et.data[1], et.data[2])]
  else:
    return getTreeAsSth(et.left) + getTreeAsSth(et.right)
  

# Less restrictive then applying as we just check if it loosely fits.
def verifyApplication(w1, eTree, word):
  
  orig_word = word
  treeParts = getTreeAsSth(eTree)
  '''
  #print(w1 + ' : ' + word)
  #print(getTreeAsSth(eTree))
  #sys.exit(0)
  allRight = True
  ...
  counterDictET = {}
  for l in w1:
    if l not in counterDictW1:
      counterDictW1[l] = 0
    counterDictW1[l] += 1
  for l in word:
    if l not in counterDictWord:
      counterDictWord[l] = 0
    counterDictWord[l] += 1
  
  counterDictET = {}
  for part in treeParts:
    if part[0] not in counterDictET:
      counterDictET[part[0]] = 0
    if part[1] not in counterDictET:
      counterDictET[part[1]] = 0
    counterDictET[part[0]] -= 1
    counterDictET[part[1]] += 1
  
  #print(counterDictET)
  #print(counterDictW1)
  #print(counterDictWord)
  for l in counterDictET:
    if l == u'':
      #print('yeah')
      continue
    if l in counterDictW1:
      w1_counter = counterDictW1[l]
    else:
      w1_counter = 0
    if l in counterDictWord:
      word_counter = counterDictWord[l]
    else:
      word_counter = 0
    
    if counterDictET[l] + w1_counter != word_counter:
      return False
...
def editTreesByPos(lang, resulting_predictions):
  
  #print(resulting_predictions)
  #sys.exit(0)
  
  # Because of adaptation reasons, lemma usually means the original word form (src).
  
  formToEt = {} # stores the possible edit trees for each tagPAIR
  lemmaToEtAndTag = {} # this is actually formToEtAndTag
  
  lemmataToForm = {} # storing all appearing forms (in tags) and words with each original form, by POS
  ...
  possiblePOS = ['V', 'ADJ', 'N']
  with codecs.open('/mounts/Users/cisintern/huiming/SIGMORPHON/Code/data/' + lang + '-task2-train', 'r', encoding="utf-8") as train_file:
    for line in train_file:
      ...
      for tag in tags1:
        if tag.split('=')[0] == 'pos':
          POS = tag.split('=')[1]
      if parts[1] not in lemmataToForm[POS]:
	# lemmataToForm[POS] has lemma -> src_tag, trg_tag, word form
        lemmataToForm[POS][parts[1]] = [(parts[0], parts[2], parts[3])]
      else:
        lemmataToForm[POS][parts[1]].append((parts[0], parts[2], parts[3]))
  ...
  for aPos in possiblePOS:
    ...
    for lemma, forms in lemmataToForm[aPos].items():
      ...
      for f1 in forms: # f1[0] is the src_tag, f1[1] is the trg_tag, f1[2] is the word form
        if f1[2] == lemma:
          continue
        else: 
          et1 = get_only_tree(lemma, f1[2])
      
          # Small test (should also never happen):
          if f1[2] != applyOnlyTree(lemma, et1):
            print("error1")
            sys.exit(0)
           
          # actually to tags
          lemmaToEtAndTag[lemma].add((et1, f1[0], f1[1]))
          
          # store (src_tag, trg_tag) to edit tree
          if (f1[0], f1[1]) not in formToEt:
            formToEt[(f1[0], f1[1])] = set()
          formToEt[(f1[0], f1[1])].add(et1)
    ...
    ''' TODO: use this if useful, however now not really
    et_count_dict = {}
    for (src_tag, trg_tag, w1), (word, prob) in resulting_predictions.iteritems():
      new_et = get_only_tree(w1, word)
      
      # Small test (should also never happen):
      if word != applyOnlyTree(w1, new_et):
        print("error1")
        sys.exit(0)

      treeString = u''
      for parts in getTreeAsSth(new_et):
	treeString += parts[0] + ':' + parts[1]
      if treeString not in et_count_dict:
	et_count_dict[(src_tag, trg_tag, treeString)] = 1
      else:
	et_count_dict[(src_tag, trg_tag, treeString)] += 1
	if et_count_dict[(src_tag, trg_tag, treeString)] >= 2:
	  # store (src_tag, trg_tag) to edit tree
          if (src_tag, trg_tag) not in formToEt:
            formToEt[(src_tag, trg_tag)] = set()
          formToEt[(src_tag, trg_tag)].add(new_et)
    '''
    
  # formToEt has (src_tag, trg_tag) -> et
  return editTrees, lemmaToEtAndTag, formToEt

if __name__ == "__main__":
  et = get_only_tree('Pferd', 'Pferde')
  print(verifyApplication('Pferd', et, 'Pferde'))
  print(verifyApplication('Schlumpf', et, 'Schlümpfe'))
  print(verifyApplication('Haus', et, 'Häuser'))
  print(verifyApplication('Blatt', et, 'Blatts'))
  sys.exit(0)
