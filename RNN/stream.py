...
def _ensure_special_tokens(vocab, bos_idx=0, eos_idx=2, unk_idx=1):
...
class _oov_to_unk(object):
    ...
    def __init__(self, src_vocab_size=3000, trg_vocab_size=3000,
                 unk_id=1):
...
class _too_long(object):
    ...
    def __init__(self, seq_len=500): # orig:50
...
def get_tr_stream(src_vocab, trg_vocab, src_data, trg_data,
    ...
    src_vocab = _ensure_special_tokens(
        ...
        bos_idx=0, eos_idx=2, unk_idx=unk_id)
...
#This is changed compared to the SIGMORPHON code.
def get_test_stream(src_vocab, trg_vocab, src_data, trg_data=None,
...
def get_tr_stream_predict(src_vocab, src_data, trg_data,
    ...
    # Pad sequences that are short
    masked_stream = PaddingWithEOS(
        stream, [src_vocab_size - 1, src_vocab_size - 1])

    return masked_stream
  
def get_dev_stream(val_set=None, src_vocab=None, config = None, src_vocab_size=3000,
                   unk_id=1, **kwargs):
    """Setup development set stream if necessary."""
    if config['val_set'] == None:
      return None
    #dev_stream = None
    #print('getting dev stream')
    
    dev_stream = get_tr_stream(config['src_vocab'], config['trg_vocab'], config['val_set'], config['val_set_grndtruth']) 
    '''
    ...
    print(dev_stream)
    maxi = 0
    for i, line in enumerate(dev_stream.get_epoch_iterator()):
      print(line[0])
      input_ = numpy.tile(line[0], (1, 1))
