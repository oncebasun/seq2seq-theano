...
class BaseSequenceGenerator(Initializable):
    ...
    **Algorithm:**
    ...
    def generate(self, outputs, **kwargs):
        ...
        #self.next_probs = self.readout.emitter.emitProbs(next_readouts)[next_outputs[-1]]
...
class NewSoftmaxEmitter(AbstractEmitter, Initializable, Random):
    ...
    def emit(self, readouts):
        ...
        self.pvals_flat = probs.reshape((batch_size, -1))
        #generated = self.theano_rng.multinomial(pvals=self.pvals_flat)
        winning_index = self.pvals_flat.reshape(probs.shape).argmax(axis=-1)
        #return generated.reshape(probs.shape).argmax(axis=-1)
        return winning_index, self.pvals_flat[0][winning_index]
...
class SamplingSoftmaxEmitter(AbstractEmitter, Initializable, Random):
    """A softmax emitter for the case of integer outputs.
    Interprets readout elements as energies corresponding to their indices.
    Parameters
    ----------
    ...
    """
    def __init__(self, initial_output=0, **kwargs):
        super(SamplingSoftmaxEmitter, self).__init__(**kwargs)
        self.initial_output = initial_output
        self.softmax = NDimensionalSoftmax()
        self.children = [self.softmax]
        self.name = 'newsoftmaxemitter'
...
class Decoder(Initializable):
    ...
    def __init__(self, vocab_size, embedding_dim, state_dim,
                 representation_dim, use_attention=True, theano_seed=None, cost_type=None, **kwargs):
	...
	readout = Readout(
        ...
        if use_attention or not use_attention:
...
class SamplingDecoder(Initializable):
    """Decoder of RNNsearch model."""

    def __init__(self, vocab_size, embedding_dim, state_dim,
                 representation_dim, use_attention=True, theano_seed=None, cost_type=None, **kwargs):
        super(SamplingDecoder, self).__init__(**kwargs)
        self.vocab_size = vocab_size
        self.embedding_dim = embedding_dim
        self.state_dim = state_dim
        self.representation_dim = representation_dim
	...
	readout = Readout(
	    source_names=['states', 'feedback',
		    self.attention.take_glimpses.outputs[0]],
	    readout_dim=self.vocab_size,
	    emitter=SamplingSoftmaxEmitter(initial_output=-1, theano_seed=theano_seed),
	    feedback_brick=LookupFeedbackWMT15(vocab_size, embedding_dim),
	    post_merge=InitializableFeedforwardSequence(
	        [Bias(dim=state_dim, name='maxout_bias').apply,
		Maxout(num_pieces=2, name='maxout').apply,
